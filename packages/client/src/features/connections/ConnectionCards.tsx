...
function ConnectionCard({ connection }: { connection: Connection }) {
  ...
  const body = (
    <>
      <ProviderIcon provider={provider} className="size-5 shrink-0 text-ink-secondary" />
  ...
  const className =
    "oc-pf-connection flex items-center gap-2.5 rounded-md border border-border bg-surface-2 px-2.5 py-2";

  if (!profileUrl) return <div className={className}>{body}</div>;
  return (
    <a
      href={profileUrl}
      target="_blank"
...
export function ConnectionCards({ connections }: { connections: Connection[] }) {
  ...
  return (
    <div className="oc-pf-connections mt-2.5 border-t border-border pt-2.5">
