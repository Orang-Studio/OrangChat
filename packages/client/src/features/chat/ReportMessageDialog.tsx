          ...
          <div className="space-y-4">
            ...
            {message.ciphertext && (
              <div className="rounded-xl border border-warning/40 bg-warning/10 p-3 text-xs text-ink-secondary">
                Reporting deliberately reveals this one decrypted message and a one-message key.
                It does not reveal the conversation key, earlier messages, or later messages.
              </div>
            )}
            <label className="block">
              <span className="text-sm font-medium">What happened? (optional)</span>
              <textarea
                value={reason}
                onChange={(event) => setReason(event.target.value.slice(0, 1000))}
                rows={4}
                placeholder="Add context that will help review this report"
